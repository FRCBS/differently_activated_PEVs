...
fviz_pca_ind(pca_data,
             ...
             legend.title = "Groups")+labs(title = "PCA of  normalized PEA data", x= paste0("PC1 (",round(eigenvalues[1,2],2),"%)"), y= paste0("PC2 (",round(eigenvalues[2,2],2),"%)")) + theme_minimal()+ scale_shape_manual(values = rep(16, length(unique(PEA_groups)))) + guides(color = guide_legend(override.aes = list(shape = 16, size = 4)))+theme(axis.title.x = element_text(size = 13, color = "black"), axis.title.y = element_text(size = 13, color = "black"), axis.text.x = element_blank(), axis.text.y = element_text(size = 11, color = "black"))
...
### Loading PEA data for top proteins and plotting boxplots and barplots 

```{r}
PEA_weights_factor1 <- get_weights(model, view = "inflammation panel", factor = 1, scale = FALSE, as.data.frame = TRUE) %>% select(-view, -factor) %>%  arrange(-abs(value))
print(PEA_weights_factor1)

```

### Barplots of the top proteins from loadings in PC1 and PC2 as well as weights for factor 1 
...
boxplot_proteins <- function(df_long, name){
  ...
  ggplot(aes(x= Group, y = NPX,  fill = Group))+ geom_boxplot() + theme_minimal()+ labs(title = name)+ theme(plot.title = element_text(hjust=0.5, size =15), axis.title.x = element_text(size = 13, color = "black"), axis.title.y = element_text(size = 13, color = "black"), axis.text.x = element_blank(), axis.text.y = element_text(size = 13, color = "black"))
}
