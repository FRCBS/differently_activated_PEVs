...
fviz_pca_ind(pca_data,
             ...
             legend.title = "Groups")+labs(title = "PCA of  normalized PEA", x= paste0("PC1 (",round(eigenvalues[1,2],2),"%)"), y= paste0("PC2 (",round(eigenvalues[2,2],2),"%)")) + theme_minimal()+ scale_shape_manual(values = rep(16, length(unique(PEVmiRNA_groups)))) + guides(color = guide_legend(override.aes = list(shape = 16, size = 3)))
...
boxplot_proteins <- function(df_long, name){
  ...
  ggplot(aes(x= Group, y = NPX,  fill = Group))+ geom_boxplot() + theme_minimal()+ labs(title = name)+ theme(plot.title = element_text(hjust=0.5, size =15))
