...
prot_long <- prot_data %>% 
  ...
  mutate(Group=rep(c("Unstimulated", "Unstimulated", "Unstimulated", "GPVI", "GPVI","GPVI", "CLEC2", "CLEC2", "CLEC2","TC","TC","TC"), times  = length(rownames(prot_data))))
head(prot_long)
...
prot_long <- clec2_proteins %>% 
  rownames_to_column(var = "Protein") %>% 
  pivot_longer(-Protein, names_to= "Sample", values_to = "Abundance")%>% 
  mutate(Group=rep(c("Unstimulated", "Unstimulated", "Unstimulated", "GPVI", "GPVI","GPVI", "CLEC2", "CLEC2", "CLEC2","TC","TC","TC"), times  = 6))
head(prot_long)
...
boxplot_proteins <- function(df_long, name){
  ...
  ggplot(aes(x= Group, y = Abundance,  fill = Group))+ geom_boxplot() + theme_minimal()+ labs(title = name)+ theme(plot.title = element_text(size =11))
}
...
#We load miRNA dataand add sample names and miRNAs as rownames
...
miRNA_long <- miRNA_data %>% 
  ...
  mutate(Group=rep(c("GPVI", "GPVI","GPVI", "GPVI", "CLEC2", "CLEC2", "CLEC2", "CLEC2","TC","TC","TC","TC", "Unstimulated", "Unstimulated","Unstimulated", "Unstimulated"), times = length(rownames(miRNA_data))))
head(miRNA_long)
...
miRNA_long <- clec2_miRNA %>% 
  rownames_to_column(var = "miRNA") %>% 
  pivot_longer(-miRNA, names_to= "Sample", values_to = "Counts")%>% 
  mutate(Group=rep(c("GPVI", "GPVI","GPVI", "GPVI", "CLEC2", "CLEC2", "CLEC2", "CLEC2","TC","TC","TC","TC", "Unstimulated", "Unstimulated","Unstimulated", "Unstimulated"), times = length(rownames(clec2_miRNA))))


#Function for creating boxplots for each sample for each protein, with boxplots colored by group
boxplot_miRNA<- function(df_long, name){
  df_long %>% filter(miRNA == name) %>% 
  ggplot(aes(x= Group, y = Counts,  fill = Group))+ geom_boxplot() + theme_minimal()+ labs(title = name)+ theme(plot.title = element_text(size =11))
}
...
boxplot_proteins <- function(df_long, name){
  df_long %>% filter(Protein == name) %>% 
  ggplot(aes(x= Group, y = NPX,  fill = Group))+ geom_boxplot() + theme_minimal()+ labs(title = name)+ theme(plot.title = element_text(size =11))
}
